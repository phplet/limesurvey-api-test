...
class remotecontrol_handle
{
        ...
        if ($this->_checkSessionKey($sSessionKey))
        {
            if (Permission::model()->hasGlobalPermission('surveys','create'))
            {
                ...
                try
                {
                    ...
                    // Load default email templates fo rthe chosen language
    ...
    */
    public function add_group($sSessionKey, $iSurveyID, $sGroupTitle, $sGroupDescription='')
    {
        if ($this->_checkSessionKey($sSessionKey))
        {
            // Author: AJ
            // Date: 09/02/16
            // Discription: Yeah yeah yeah. Cut me a break
    ...
    * @param string $sLanguage Optional parameter language for multilingual questions
     * @param string $test tests if the question property can be changed (used for moving between groups)
    * @return array Of succeeded and failed modifications according to internal validation.
    */
    public function set_question_properties($sSessionKey, $iQuestionID, $aQuestionData,$sLanguage=NULL,$test=NULL)
    {
        if ($this->_checkSessionKey($sSessionKey))
        {
            Yii::app()->loadHelper("surveytranslator");
            ...
            if (Permission::model()->hasSurveyPermission($iSurveyID, 'survey', 'update'))
            {
                ...
                // makes sure that the survey is not active if they want to change the group id
                $oSurvey=Survey::model()->findByPk($iSurveyID);
                if($oSurvey->getAttribute('active') == 'Y' && $aQuestionData['gid']) { return 'Error: Survey is active'; }
                ...
                foreach($aQuestionData as $sFieldName=>$sValue)
                {
                    ...
                    if((isset($dependencies) || isset($is_criteria_question))  && $sFieldName == 'question_order')
                    {
                        ...
                        // continue;
                        return array('status' => 'Error: Questions with dependencies can not be changed');
                    }
    ...
    */
    public function add_participants($sSessionKey, $iSurveyID, $aParticipantData, $bCreateToken=true)
    {
        ...
        if (Permission::model()->hasSurveyPermission($iSurveyID, 'tokens', 'create'))
        {
            ...
            $a = array();
            foreach (json_decode($aParticipantData,true) as $key => $value)
            {
                $token = Token::create($iSurveyID);
                $a = array_intersect_key($value,$aDestinationFields);
                $aParticipantDataAsArray = json_decode($aParticipantData,true);
                $token->setAttributes(array_intersect_key($aParticipantDataAsArray[$key],$aDestinationFields));
                if  ($bCreateToken)
                {
                    $token->generateToken();
                }
                if ($token->save())
                {
                    $value = $token->getAttributes();
                }
                else
                {
                    $value["errors"] = $token->errors;
                }
            }
            return $value;
        }
                ...
                foreach ($oTokens as $token)
                {
                    $aTempData = array(
                        ...
                        'participant_info'=>array(
                            ...
                            'sent'=>$token->attributes['sent'],
                            'remindedsent'=>$token->attributes['remindersent'],
                            'remindercount'=>$token->attributes['remindercount'],
                            'completed'=>$token->attributes['completed']
                        )
                    );
            ...
            if (Permission::model()->hasSurveyPermission($iSurveyID, 'survey', 'read'))
            {
                ...
                foreach ($aQuestionList as $oQuestion)
                {
                    $aData[]= array(
                        ...
                        'group_id'=>$oQuestion->attributes['gid'],
                        'title'=>$oQuestion->attributes['title'],
                        'type'=>$oQuestion->attributes['type'],
                        'question'=>$oQuestion->attributes['question'],
                        'help'=>$oQuestion->attributes['help'],
                        'order'=>$oQuestion->attributes['question_order'],
                        'mandatory'=>$oQuestion->attributes['mandatory'],
                        'relevance'=>$oQuestion->attributes['relevance']
    ...
    */
    public function list_surveys($sSessionKey, $sUsername=NULL)
    {
        ...
        {
            $oSurvey = new Survey;
            if (!Permission::model()->hasGlobalPermission('surveys','read') && ($sUsername == null))
            {
    ...
    */
    ...
    // (doesn't work without $sSessionKey for some reason)
    public function api_exists($sSessionKey)
    {
        return array('status' => 'Lime Survey API end point');
    }


    public function session_key_is_valid($sSessionKey)
    {
        if($this->_checkSessionKey($sSessionKey)) {
            return array('status' => 'Session key is valid');
        } else {
            return array('status' => 'Invalid session key');
        }
    }
    ...
    public function duplicate_survey($sSessionKey, $iSurveyID)
    {
        ...
        foreach ($surveys as $survey) {
            if ($survey['surveyls_title'] == $new_survey_name) { return array('status' => 'Error: A survey with that name already exists'); }
        }
        ...
        if (!Survey::model()->findByPk($iSurveyID)) { return array('status' => 'Error: Invalid survey ID'); }
        elseif (!Permission::model()->hasSurveyPermission($iSurveyID, 'surveycontent', 'export')) { return array('status' => 'No permissions'); }
        else {
    ...
    /**
     * RPC Routine to create a question
     *
     * @access public
     * @param string $sSessionKey - Auth credentials
     * @param int $iSurveyID - the survey we are going to add the question to
     * @param string $question - the text of the question we want to ask
     * @param string $help_text - 
     * @param string $type - the type of question we are going to add
     * @param string $mandatory - if the question is mandatory or not
     * @param string $other - whether and empty text area called 'other' should be added to the question
     * 
     * */
    ...
    public function add_question($sSessionKey, $iSurveyID, $question, $help_text, $type, $mandatory, $other) {
        ...
        // create the question
            ...
            $new_question_title = 'Q' . ($total_questions + 1);
     ...
     * @param string $data - the order of the questiosn in the below format
     * 
     * */
    // NOTE: Submit data in this format: list[g517]=root&list[q5204]=g517&list[q5205]=g517&list[q5196]=g517&list[q5192]=g517
    public function order_questions($sSessionKey, $iSurveyID, $data) {
        $AOrgData = array();
        parse_str($data, $AOrgData);
        $grouporder = 0;
        ...
        return "The new question order was successfully saved";
    }

    /**
     * RPC Routine to get questions answer items
     *
     * @access public
     * @param string $sSessionKey - Auth credentials
     * @param string $iSurveyID - Survey ID
     * @param string $iQuestionID - Question ID
     * 
     **/
    public function get_question_answers($sSessionKey, $iSurveyID, $iQuestionID) {
        $output = array();
    ...
    /**
     * RPC Routine to update a questions answer items
     *
     * @access public
     * @param string $sSessionKey - Auth credentials
     * @param string $iSurveyID - Survey ID
     * @param string $iQuestionID - Question ID
     * @param string $data - the order of the sub answers in the below format
     * 
     **/
    // NOTE: Submit data in this format:
    //   sid=387681&gid=517&qid=5204&oldcode_1_0=1&code_1_0=1&
    //   assessment_1_0=0&answer_en_1_0=Clean&oldcode_2_0=2&code_2_0=2&assessment_2_0=1&
    //   answer_en_2_0=Normal&oldcode_3_0=3&code_3_0=3&assessment_3_0=1&answer_en_3_0=Dirty&
    //   code_4_0=4&assessment_4_0=1&answer_en_4_0=jjj&answercount_0=5&bFullPOST=1
     ...
     * 
     **/
    public function get_question_sub_questions($sSessionKey, $iSurveyID, $iQuestionID) {
        $output = array();
        foreach (Question::model()->findAllByAttributes(
            array(
                'sid'        => $iSurveyID,
                //'gid'        => $iQuestionGroupID,
                'parent_qid' => $iQuestionID,
                'language'   => Survey::model()->findByPk($iSurveyID)->language
                //'scale_id'   => 0
            )
        ) as $sub_question) {
            $output[] = array(
                'sid' => $sub_question->sid,
                'gid' => $sub_question->gid,
                'qid' => $sub_question->qid,
                'parent_qid' => $sub_question->parent_qid,
                'code' => $sub_question->title,
                'question_order' => $sub_question->question_order,
     ...
     **/
    // NOTE: Submit data in this format:
    //   deletedqids=+5210+5209&oldcode_5193_0=SQ001&code_5193_0=SQ001&answer_en_5193_0=Good&
    //   oldcode_5194_0=SQ002&code_5194_0=SQ002&answer_en_5194_0=Average&oldcode_5195_0=SQ003&
    //   code_5195_0=SQ003&answer_en_5195_0=Bad&method=Save+changes&bFullPOST=1
    public function set_question_sub_questions($sSessionKey, $iSurveyID, $iQuestionID, $data) {
        ...
        foreach ($aDeletedQIDs as $iDeletedQID) {
            ...
            if($iDeletedQID > 0) { // don't remove undefined
                ...
                if (!$iInsertCount) { return "Failed to delete answer"; }
            }
        }
                    ...
                    if($bSubQuestionResult) {
                        if(substr($subquestionkey,0,3) != 'new' && isset($aOldCodes[$iScaleID][$iPosition]) && $aCodes[$iScaleID][$iPosition] !== $aOldCodes[$iScaleID][$iPosition]) {
                            Condition::model()->updateAll(
                                array(
                                    'cfieldname'=>'+'.$iSurveyID.'X'.$iQuestionGroupID.'X'.$iQuestionID.$aCodes[$iScaleID][$iPosition],
                                    'value'=>$aCodes[$iScaleID][$iPosition]
                                ),
                                'cqid=:cqid AND cfieldname=:cfieldname AND value=:value',
                                array(
                                    ':cqid'=>$iQuestionID,
                                    ':cfieldname'=>$iSurveyID.'X'.$iQuestionGroupID.'X'.$iQuestionID,
                                    ':value'=>$aOldCodes[$iScaleID][$iPosition]
                                )
                            );
                        }
                        if (!isset($aInsertQID[$iScaleID][$iPosition])) {
     ...
     * @param string $sSessionKey - Auth credentials
     *
     * (only used internally, not exposed to the front end)
     * 
     * */

    public function get_user_id($sSessionKey) {
        // Check sessionkey
        if (!$this->_checkSessionKey($sSessionKey)) { return array('status' => 'Invalid session key'); }
        // return theuser id
        return array('status' => Yii::app()->session['loginID']);
    }


    /**
     * RPC Routine to search all the participants in the Central Participants Database
     *
     * @access public
     * @param string $sSessionKey - Auth credentials
     * 
    ...
    public function search_global_participants($sSessionKey, $value, $page = null) {
        if($page == null) { $page = 1; }
        // Cut'n'pate from > applications/controllers/admin/participantsaction.php
        // SEARCH EMAIL ADDRESSES
        $email_search_term = array('email', 'contains', $value);
        $email_search = Participant::model()->getParticipantsSearchMultipleCondition($email_search_term);
        $email_search_array = $this->get_global_participants($sSessionKey, $email_search, $page);
        // SEARCH FIRST NAME
        $firstname_search_term = array('firstname', 'contains', $value);
        $firstname_search = Participant::model()->getParticipantsSearchMultipleCondition($firstname_search_term);
        $firstname_search_array = $this->get_global_participants($sSessionKey, $firstname_search, $page);
        // SEARCH LAST NAME
        $lastname_search_term = array('lastname', 'contains', $value);
        $lastname_search = Participant::model()->getParticipantsSearchMultipleCondition($lastname_search_term);
        $lastname_search_array = $this->get_global_participants($sSessionKey, $lastname_search, $page);
        // merge all the results
        $return_array = array();
        if($email_search_array->rows != null) { array_push($return_array,$email_search_array->rows); }
        if($firstname_search_array->rows != null) { array_push($return_array,$firstname_search_array->rows); }
        if($lastname_search_array->rows != null) { array_push($return_array,$lastname_search_array->rows); }
        return array(
            'records' => count($email_search_array->rows) + count($firstname_search_array->rows) + count($lastname_search_array->rows),
            'total'   => 1,
            'page'    => 1,
            'rows'    => $return_array
        );
        return array_merge($email_search_array->rows,$firstname_search_array->rows,$lastname_search_array->rows);
    }
    ...
    public function get_global_participants($sSessionKey, $search = null, $page = null, $results_per_page = 1000000) {
        ...
        $limit = isset($limit) ? $limit : 50; //Stop division by zero errors (don't know if this is still needed if it is populated in the function call)
        ...
        foreach ($attid as $key => $value) { array_push($participantfields, $value['attribute_id']); }
        // sorts the array
        // TODO: 
        ...
        if (Permission::model()->hasGlobalPermission('superadmin','read')) { $iUserID = null; }
        else { $iUserID = Yii::app()->session['loginID']; }
        ...
        {            
            if (array_key_exists('can_edit', $row)) {
                ...
                if (is_null($sCanEdit)) { $sCanEdit = 'true'; }
            // Super admin
            } else { $sCanEdit = "true"; }
    ...
    /**
     * RPC Routine to edit a participant in the Central Participants Database
     *
     * @access public
     * @param string $sSessionKey - Auth credentials
     * @param string $participantID - the PK in the DB for the participant
    ...
    public function set_global_participant($sSessionKey, $participantID, $first_name, $last_name, $email, $blacklisted = 'N', $language = 'en') {
        // Check sessionkey
        if (!$this->_checkSessionKey($sSessionKey)) { return array('status' => 'Invalid session key'); }
        // Cut'n'pate from > applications/controllers/admin/participantsaction.php
        $aData = array(
        ...
        //Yii::log(var_export($aData,true), 'info', 'a.b.c');
    }
    ...
    public function add_global_participant($sSessionKey, $firstName, $lastName, $email, $blacklisted = 0, $language = 'en') {
        // Check sessionkey
        if (!$this->_checkSessionKey($sSessionKey)) { return array('status' => 'Invalid session key'); }
        // Cut'n'pate from > applications/controllers/admin/participantsaction.php
        $uuid = $this->gen_uuid();
        ...
        if(Participant::model()->insertParticipant($aData)) {
            return $aData;
        } else {
            return array('status' => 'Error: The participant was not saved');
        }
    }

    // needed above
    // needed for $this->add_global_participant
    ...
    public function set_question_group($sSessionKey, $group_id) {
        ...
        if (!$this->_checkSessionKey($sSessionKey)) { return array('status' => 'Invalid session key'); }
        // Check permissions
        if (Permission::model()->hasSurveyPermission($surveyid, 'surveycontent', 'update')) { return array('status' => 'No permission'); }
    ...
    public function edit_answer($sSessionKey, $group_id) {
/*        if ($sAction == "updateansweroptions" && Permission::model()->hasSurveyPermission($iSurveyID, 'surveycontent','update'))
    ...
    /**
     * RPC Routine to invite a single participant to survey
     *
     * @access public
     * @param string $sSessionKey Auth credentials
     * @param int $iSurveyID ID of the survey that participants belong
     * @return int $iTokenID ID of the token we want to send the invitiation to
     */
    public function invite_participant($sSessionKey, $iSurveyID, $iTokenID)
    {
        Yii::app()->loadHelper('admin/token');
        if (!$this->_checkSessionKey($sSessionKey))
            return array('status' => 'Invalid session key');
        ...
        if (Permission::model()->hasSurveyPermission($iSurveyID, 'tokens', 'update')) {
            ...
            if($token) {
                emailTokens($iSurveyID,array($token),'invite');
                return $iTokenID;
            } else {
                return 'Error: Invitation already sent';
            }
        } else { return array('status' => 'No permission'); }
    }
